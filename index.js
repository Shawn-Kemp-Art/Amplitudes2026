        ...
        for (var z = 0; z < stacks; z++) {
            if (z < stacks - 1) {
                for (var zs = z + 1; zs < stacks; zs++) {
                    var old = sheet[z];
                    sheet[z] = clipSubtract(old, sheet[zs]);
                    old.remove();
                }
            }
        }
...
function circleFrame(z){
    ...
    path2 = clipSubtract(outsideframe, path);
    outsideframe.remove();path.remove();
    var old = sheet[z];
    sheet[z] = clipUnite(old, path2);
    old.remove();
    path2.remove();
}
...
function solid(z){
    ...
    var old = sheet[z];
    sheet[z] = clipUnite(old, outsideframe);
    old.remove();
    outsideframe.remove();
}
...
function frameIt(z,round){
    ...
    var old = sheet[z];
    sheet[z] = clipIntersect(outsideframe, old);
    outsideframe.remove();
    old.remove();
